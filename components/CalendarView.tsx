...
export function CalendarView({ selectedDate, onDateChange, loggedFoods }: CalendarViewProps) {
  ...
  return (
    <div className="w-full p-4 space-y-8 bg-gray-50 min-h-screen">
      {/* Calendar */}
      <div className="rounded-xl border border-gray-300 bg-white shadow p-6">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={previousMonth} className="rounded-md p-2 hover:bg-blue-100" aria-label="Previous month">←</button>
            <button onClick={goToToday} className="rounded-md px-3 py-1 text-sm font-medium bg-blue-500 text-white hover:bg-blue-600">Today</button>
            <button onClick={nextMonth} className="rounded-md p-2 hover:bg-blue-100" aria-label="Next month">→</button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-7 gap-2">
          {days.map((date, idx) => (
            <button
              ...
              disabled={!date}
              className={`relative flex items-center justify-center h-12 w-12 rounded-md text-sm font-medium transition-colors ${
                !date ? "bg-gray-100 cursor-default text-gray-400" :
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-lg bg-green-50 text-center shadow">
          <p className="text-sm font-medium text-gray-600">Daily Avg</p>
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-end justify-between h-64 gap-2">
            {weeklyData.map((day, idx) => (
              <div key={idx} className="flex flex-col items-center gap-1">
                ...
                <div
                  className="w-6 rounded-t-md bg-red-500 transition-all hover:opacity-80"
        ...
        <div className="p-6 rounded-lg bg-white shadow space-y-4">
          <h3 className="text-lg font-semibold text-gray-800">Macro Distribution</h3>
          {["protein","carbs","fat"].map((macro, idx) => {
            const color = macro === "protein" ? "bg-blue-500" : macro === "carbs" ? "bg-green-500" : "bg-amber-500"
            return (
              <div key={idx}>
                <div className="mb-1 text-sm font-medium capitalize text-gray-700">{macro}</div>
                <div className="flex h-8 items-end gap-1 rounded-md bg-gray-200 p-1">
                  {weeklyData.map((day, i) => (
                    <div
                      ...
                      style={{
                        height: `${Math.max((day[macro as keyof DailyStats] as number / maxMacro) * 100, 4)}%`,
                      }}
                      title={`${day.day}: ${day[macro as keyof DailyStats]}g`}
                    />
                  ))}
                </div>
              </div>
