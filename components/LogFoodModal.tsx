...
interface LogFoodModalProps {
  onClose: () => void
  onAddFood: (foodItems: FoodItem[]) => Promise<void>  // FIX: Allow async for Firestore/local ops
  initialTab: ModalTab
}
      ...
      return (
        <div className="space-y-4">
          ...
          <div className="flex gap-3 sm:gap-4 mt-4">
            ...
            <button
              onClick={async () => {  // OPTIONAL: Add async wrapper for better UX (e.g., temp loading)
                if (analysisResult.length > 0) {
                  await onAddFood(analysisResult);  // Now async-safe
                  onClose();  // Close modal after add (if desired)
                }
              }}
